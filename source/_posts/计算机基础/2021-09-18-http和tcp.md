...
![avatar](../../assets/images/net/3.png)
...
* **主动关闭方**在收到被动关闭方的FIN包后并返回ACK后，会进入TIME_WAIT状态
...
* 客户端等待两个最大数据段生命周期(Maximum segment lifetime，MSL)*2的时间后也会进入 `CLOSED` 状态。这样可以防止新连接创建数据包和老链接数据包错乱, 相当于禁止客户端创建新连接一段时间(直到老链接数据包丢弃完毕)。
...
### 同步和阻塞, Reactor和Proactor

同步和异步是进程，进程需要等待(阻塞)称为同步, 进程不需要等待称为异步。**同步往往指的是用户进程触发IO操作并等待(epoll_wait)或者轮询的去查看IO操作是否就绪**，而异步是指用户进程触发IO操作以后便开始做自己的事情，而当**IO操作已经完成的时候会得到IO完成的通知**。

阻塞和非阻塞是进程访问数据，阻塞方式下读取或者写入函数期间将一直等待知道的读写完，而非阻塞方式下，读取或者写入函数会立即返回一个状态值。

需要注意为什么可以异步, 因为**本该线程同步等待的变成了操作系统线程等待**, 得到完成发送通知。操作系统不是万能的, 而且这种办法和开俩线程, 一个等待, 一个作别的事情没太多区别。所以, 异步不见得比同步好。同步多维护一个线程也可以实现类似异步的效果, 而且可控可修改, 毕竟操作系统bug也没法改。

* 同步阻塞， 用户进程在发起一个IO操作以后，必须等待IO操作的完成，进程需要等待数据到来以及读写数据
* 同步非阻塞IO：在此种方式下，用户进程必须等待数据事件触发, 但读写数据时不必阻塞写完
* 异步阻塞IO：进程不必等待数据触发, 事件触发时会通知该进程, 但进程读写数据时依然是阻塞。
* 异步非阻塞IO: 进程不必等待事件触发, 事件触发时会通知进程; 也不必等待数据写完(数据复制被其他线程做), 轮询查看数据是否准备完或者数据准备完通知进程。

在socket的选项设置`O_NONBLOCK`，这样对于该socket的`send()`或`recv()`便会采用非阻塞方式。注意到Reactor用于同步IO(可以是阻塞或者非阻塞)，Proactor用于异步IO。例如Reactor模式下使用select, 同步非阻塞的方法是用一个线程监听多个文件描述符, 同步阻塞的方法往往是开多个线程每个监听一个连接。
...
Reactor模式和Proactor模式都是是event-driven architecture（事件驱动模型）的实现方式, 当连接到来, 消息到来等会触发事件(一般就是读写事件), 通过监听事件和处理事件实现服务端的通信。读写数据一般都是非阻塞。
