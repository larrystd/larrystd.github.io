...
主从复制, master/slave, 复制功能分为同步sync和命令传播command propagate。

同步基于SYNC命令和RDB文件复制。 PSYNC增加了完整重同步和部分重同步, 完整重同步主要应用初次开机复制, 部分重同步断线重连。主服务器的复制缓冲区backlog记录某个从服务器当前的同步状态

哨兵sentiel, 作用是监控master以及failover, 提供高可用性。监控的方式就是通过心跳。每个sentiel作为节点维护sentielState结构体, 里面有所监控各master的状态

主观下线，客观下线，选举是过半选举。选举出的sentiel负责failover

cluster集群，集群是统筹主从复制和哨兵的高可用解决方案, 哨兵可用来监控状态但failover不用它了。

cluster有分片逻辑, 也就是slot共16384个, 槽被指派到节点, 指派信息通过位图描述。一个节点挂了slot会进行重新分片。

集群中的master负责处理slot(写, 元数据, slot重新分片等), 从节点用于复制主节点和failover。故障检测通过节点心跳, 选举节点和raft算法类似, epoch投票。

自动计算key存在哪个slot可以让master减轻存储元数据的压力, 读数据也不用访问master记录数据存在哪了