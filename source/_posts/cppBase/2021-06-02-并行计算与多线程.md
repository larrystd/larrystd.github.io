---
title: 多线程和并行pthread, thread, mpi
date: 2021-06-03
updated: 2021-06-03
tags: 
  - cpp
...
void* xc(void* arg){    // 函数格式注意
 ...
 void main(){
    ...
    void *status;
    pthread_join(tid,&status);  // 主线程等待tid的子线程
    printf("返回%d\n",(int)status);
}
```
...
```
显然这个私有变量实际只是key-val对象, 使用繁琐, 效率低, 一般不这么用,而是用__thread。
...
__thread被GNU支持,clang等支持, 可以认为是C语言表示线程私有变量的形式, __thread和thread_local之间的不同就是thread_local在访问变量的线程中使用延迟初始化来初始化变量, 而__thread不初始化修饰的变量,必须手动初始化。如果修饰的是一个class, thread_local会调用构造函数, __thread则不需要。
