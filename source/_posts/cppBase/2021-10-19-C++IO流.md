...
iostream 标准库提供了 cin 和 cout 方法分别用于从标准输入读取流和向标准输出写入流(也是C++最常用的用户输入输出对象)。流对象需要重载`<<`和`>>`两个操作符, `<<`流输出数据,写入磁盘等, `>>`向流中读入数据,写入内存。注意标准输入输出对应的终端和磁盘含义一样，因此使用`cout << "123"`表示输出数据`"123"`到标准终端。
...
如果我们不打算从终端输入字符, 而是从文件中读取, 可以将cin重定向到文件中。
