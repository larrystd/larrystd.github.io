...
```
那么远程服务器127.0.0.1:12345将成为127.0.0.1:6379的从服务器, 复制中的主从服务器双方的数据库将保存相同的数据
```
// 主服务器执行
...
Redis主从复制采用的是"同步+命令传播"机制完成主从数据同步。同步操作用于将从服务器的数据库状态更新至主服务器所处的数据库状态, 命令传播操作则保证实时下主服务器的数据库状态被修改, 让主从服务器状态回到一致状态。

当客户端向服务器发送SLAVEOF命令, 从服务器首先执行同步操作。同步操作一般需要向主服务器发送SYNC命令完成, 基于RDB日志
1. 从服务器向主服务器发送SYNC命令
2. 收到命令的主服务器执行BGSAVE, 后台生成一个RDB文件, 建立一个缓冲区记录从现在开始的所有写命令
...
这里的硬伤是从机重连后，哪怕主从之间只有少量的数据不一致，也要执行一个耗时、耗资源的全量同步操作(即RDB恢复)来达到数据一致。Redis的优化是判断是否只有少量数据不一致, 如果是, 则使用增量不同, 反之使用全量同步, 基于思想是复制积压缓冲区, 用来存储主节点到从节点传递的命令。同时使用offset表示缓冲区中已经使用的内容。如果积压缓冲区内容有效, 则直接复制缓冲区不用全量同步。同时为了让从服务器识别主服务器, 每个Redis服务器都会有自己的运行ID, 由16个随机的十六进制字符组成, 当从服务器执行复制时, 它要确认当前主服务器是否为断线前的那个。


增量同步过程
![redis](../../assets/images/DB/52.png)
复制命令用PSYNC代替了SYNC, 具有完整重同步和部分重同步两种模式, 完整重同步用于初次复制, 和SYNC步骤基本一样。部分重同步用于处理断线后重复制, 基本原理就是主从服务器的复制偏移量(replication offset), 复制积压缓冲区(replication backlog), 服务器运行id。

复制积压缓冲区是master维护的固定长度(fixed size)的先进先出(FIFO)的队列, 默认大小为1MB。当master进行命令传播时, 不仅将写命令发送给follower, 还会写入复制积压缓冲区中。

如下, backlog_off之前的数据被删除, 只有同步数据的范围位于backlog_off和offset之间才能使用增量同步, 否则就是全量同步
![redis](../../assets/images/DB/50.png)

#### 命令传播
命令传播十分简单, 也就是是动态自动触发。 当master执行完新的写命令后，master会将写命令发送给follower, 同时也会把该命令追加至复制积压缓冲区。follower接收命令并执行，同时更新follower维护的复制偏移量offset。这样就可以保证任意时刻主从服务器的一致性了。
...
值得注意的是redis sentinel选举算法就是raft leader的选举算法, 参见https://larrystd.site/computerbase/2022-01-01-docker,%20k8s/, 但是raft利用复制日志保证一致性, redis没有使用, 而是通过上述的主从复制保证一致性。此外只有sentinel具有选举的权利(slave节点果然是slave啊,没有选举权)。完整的Raft保证了一致性和可用性。
