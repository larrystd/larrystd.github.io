...
```
四种新式转型各有不同的目的
- const_cast将对象的常量性转除，即const转为non-const。const的转型对象必须是指针,引用。
...
- static_cast用来强制隐式转换，例如将non-const转换成const对象，int转为double。
...
```
事实上，任何一个类型转换，不管是通过转型操作进行的显式转换，或通过编译器完成的隐式转换，都会令编译器编译出一些运行期执行的码。尽量少用转型操作，用其他方式来代替转型，尤其是向下转型dynamic_cast。
