...
lea是load effective address的缩写，简单的说，lea指令可以用来将一个内存地址直接赋给目的操作数，
...
C语言函数参数采用自右向左的入栈顺序。即在实现上, 靠右的参数先入栈。至于变量定义，就是在栈或者堆上开辟空间，所谓赋值或者初始化，就是在空间上进行赋值。对应的, Cpp STL的allocator用来申请空间, Construct用来对申请来的空间赋值。
...
1. 每个寄存器的用途并不是单一的。
2. %rax 通常用于存储函数系统调用
3. %rsp 是堆栈指针寄存器，通常会指向当前栈帧的栈顶位置，堆栈的 pop 和push 操作就是通过改变 %rsp 的值即移动堆栈指针的位置来实现的。
4. %rbp 是栈帧指针，用于标识当前栈帧的起始位置。%rbp和%rsp一起定位当前的位置, 相当于栈帧位置+偏移量
...
_start:
    ...
    xor rdi, rdi    ; 相当于mov rdi 0 传递0给rdi, return 0
    syscall
...
glibc不属于linux内核的一部分, 也不是一起维护, 因此有可能存在兼容性问题。glibc可以理解为c语言的库函数, 而boost为c++的库函数。
...
用户态程序通过软中断指令`int 0x80`(0x80号系统调用) 来陷入内核态执行系统调用, 软中断和硬中断都是一种信号, 硬中断由磁盘, 键盘, 网卡等引起, 软中断由内核引起。
...
### gdb 调试汇编
注意，只有`g++` 设置`-g`，或者设置debug模式才能进行调试，release模式不可以。gdb可以调试C语言,C++, 汇编。

#### 常用命令
1. r, Run的简写，运行被调试的程序。   如果此前没有下过断点，则执行完整个程序；如果有断点，则程序暂停在第一个可用断点处。
2. c, Continue的简写，继续执行被调试程序，直至下一个断点或程序结束。
3. b, Breakpoint的简写，设置断点。
...
si命令类似于s命令，ni命令类似于n命令。所不同的是，这两个命令(si/ni)所针对的是汇编指令, 执行一行; 而s/n针对的是源代码执行1行。
6. i
info的简写，用于显示各类信息
...
2. 直接查看内存内容,基于地址。通过, examine(简写为x)可以用来查看内存地址中的值
```
x/[n][f][u] addr
