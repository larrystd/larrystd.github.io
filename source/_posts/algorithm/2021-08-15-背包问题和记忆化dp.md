...
明确两点，**状态**和**选择**。01背包状态有两个，就是**背包的容量**和**可选择的物品**。选择就是**装进背包**或者**不装进背包**。 即01两种选择。
...
二维数组`dp[i][j]`, 表示处理物品1~i, 容量为j的前提下, 背包所能装载的最大价值。其实j有选择还是不选i物品的含义, 选i则有`dp[i][j] = dp[i-1][j-w[i]] + v[i]`, 不选i则有`dp[i][j] = dp[i-1][j]`
...
void test_2_wei_bag_problem1() {
    ...
    for (int i = 1; i < w.size(); i++)  // 从第二个物品开始考虑
    {
