...
注意configure之后还生成了重要的ngx_modules.c文件。核心是两个数组的定义, 数据内容是module的地址。
```cpp
...
ngx_command_t定义模块的配置文件参数，用来将模块参数和nginx.conf文件联系起来
```cpp
...
内存池的逻辑就是预先分配一块内存(相当于malloc), 而且你要注意销毁内存池(内存池不意味着不进行内存管理了 )
```cpp
ngx_pool_t *
...
模块声明位于./configure生成的ngx_module.c文件, ngx_module_t是模块结构体, 包含模块的基本信息。

ngx_module_t有一个成员ngx_command_t, 记录模块的配置文件信息。
```cpp
ngx_module_t *ngx_modules[] = {
    &ngx_core_module,
    &ngx_errlog_module,
...
主配置文件nginx.conf, 由三个部分组成, 全局块, events, http。http块中又包含http全局块, server块, server块可包含server全局块和location块

一些数据结构, ngx_queue_t, ngx_array_t, ngx_list_t, ngx_rbtree_t, ngx_hash_t, 

值得注意的是C语言中没有bool型, 使用冒号表示占1bit
```cpp
    unsigned         in_file:1;
    unsigned         flush:1;
    unsigned         sync:1;  // 只有一个bit
```