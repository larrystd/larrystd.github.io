...
* 选择从bagWeight到`weight[i]`从大到小遍历时为了保证添加一次(计算大的时候，前面没有被覆盖)，如果从小到大遍历后面的可能用到已经添加的数。需要考虑`dp[j]`被覆盖问题。
...
* 01背包问题的递推方程式 `dp[i][w] = max(dp[i - 1][w], dp[i - 1][w - w[i]] + v[i]); `
* 完全背包问题的递推方程式 `dp[i][w] = max(dp[i - 1][w], dp[i - 1][w - k * w[i]] + K * v[i]);`
...
for(int i = 1; i <= N; i++){
    for(int j = 1; j <= W; j++){
        for(int k = 1; j - k*w[i] >= 0; k++){ //在拿了k件的条件下容量足够
...
* 完全背包问题还有一个简单又有效的优化，那就是如果 `w[a] > w[b] && v[a] < v[b]` 这种情况下就可以a物品去掉，因为有b就没必要去选a了，因为a比b重而且a的价值比b还小。即`dp[i][j] = max(dp[i - 1][j], dp[i][j - w[i]] + v[i])`, 相比于0-1背包，只在`dp[i-1]`换成`dp[i]`
