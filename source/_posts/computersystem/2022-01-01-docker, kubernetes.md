...
![docker](../../assets/images/ceph/20.png)

An image is a read-only template with instructions for creating a Docker container. To build your own image, you create a Dockerfile with a simple syntax for defining the steps needed to create the image and run it.  Image存储了应用的静态指令代码和需要资源等, 就差运行了

A container is a runnable instance of an image. You can create, start, stop, move, or delete a container using the Docker API or CLI. 运行中的image称为container

```
docker run -i -t ubuntu /bin/bash
```

https://github.com/soh0ro0t/docker-source-analysis
...
1. kube-apiserver , exposes the Kubernetes API. 该api用于控制组件发布命令控制pod
2. etcd, Consistent and highly-available key value store used as Kubernetes' backing store for all cluster data. 一致性存储元数据, key-value
3. kube-scheduler, 调度器, watches for newly created Pods with no assigned node, and selects a node for them to run on.
4. kube-controller-manager,控制器 Control plane component that runs controller processes. 例如Node controller(节点开启或关闭), Job controller (任务控制), Service Account & Token controllers (访问控制)
...
2. kube-proxy, kube-proxy maintains network rules on nodes.

3. Container runtime, 

#### node

create a Node from the following JSON manifest
```
{
  "kind": "Node",
  "apiVersion": "v1",
...
Control Plane Components的kube-apiserver可以和node进行通信, 方式是通过https协议, kube-apiserver监听在443端口, pod发起连接, 它们都有系统设置的虚拟ip地址. As a result, the default operating mode for connections from the nodes and pods running on the nodes to the control plane is secured by default and can run over untrusted and/or public networks.

![kubernetes](../../assets/images/ceph/19.png)

#### Pod

Pods are the smallest deployable units of computing in Kubernetes. A Pod is a group of one or more containers, with shared storage and network resources, and a specification for how to run the containers. 
...
an application running on a set of Pods as a network service. 一些运行着应用程序的Pods可以认为是一个network service. A Service in Kubernetes is a REST object, similar to a Pod. Like all of the REST objects, you can POST a Service definition to the API server to create a new instance. 
...
kubernetes执行一般过程,
![kubernets](../../assets/images/ceph/21.png)

creating a resource, take an example where we are creating a pod using a command like `kubectl run some-pod --image=some-image`

![kubernets](../../assets/images/ceph/22.png)

1. Pod is created (with no node assigned) and this data is set into the etcd. A confirmation is sent back to the user stating that the pod is created.

2. Kube-scheduler monitors the api-server to find new requests, Once a node is found, it passes the information to the apiserver which feeds the same into the etcd. 监视, 当准备api-server发送命令创建pod时还会写入etcd

3. 当创建pod完毕后, Pod status is stored back in etcd.
### etcd

在Kubernetes世界中，etcd用作服务发现的后端，并存储集群的状态及其配置。由于etcd是一致性的且高可用, 因此可以存储配置文件以供Control Plane Components访问, 同时用户可以访问etcd得到所需的服务, 由于etcd是一致性且分布式, 可以扛住大量的用户访问。
