...
* Slab Class, 特定大小的chunk的组, memcached默认情况下下一个slab的最大值为前一个的1.25倍, 使用slab减少的外部碎片。当chunk不够的时候, slab申请内存时以page为单位，都会有1M大小的page被分配给该slab

memcached的内存处理比较粗糙, 和SGI STL的空间配置器差不多, 仅仅是降低了malloc由于brk的内存碎片而已, 但没有解决内部碎片的问题。相比tcmalloc, jemalloc性能差很多。
