...
声明, 只需要记住golang的数组为`[]int`而不是`int[]`写法就行了, 其他写法改动不大。如`int[3] {1,2,3}`改为`[3]int {1,2,3}`
```
var a[3] int
...
#### map
map 是引用类型, 注意用 make 来构造 map。`make(map[string]int)`
...
go test -v -run TestHello 只运行指定的单元测试用例
```