...
struct ngx_cycle_s {
  ngx_pool_t *pool; // 内存池
  ngx_log_t *log; // 日志模块
  ngx_connection_t *free_conections; // 可用连接池
  ngx_uint_t free_connection_n; // 可用连接池的总数

  ngx_connection_t *connections; // 指向当前进程所有连接对象
  ngx_uint_t connection_n; // 当前进程所有连接对象的总数
