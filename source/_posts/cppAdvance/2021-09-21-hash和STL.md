...
hashtable, buckets在开链法中是一个链表

负载因子, hashtable元素个数与buckets数量之比。rehash的扩容规则是在给定的素数表中大于当前buckets的下一个素数。

hashtable的查找, 插入, 删除效率都是O(1), 缺点是key无序, 以及负载因子太大了之后可能要rehash
