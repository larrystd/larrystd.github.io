...
* input_iterator 输入迭代器(只读迭代器), output_iterator 输出迭代器(只写迭代器) 可operator++

输入迭代器可以读取元素, 输出迭代器可以写元素, 必须支持, 

用于比较两个迭代器相等不相等的操作符operator==, operator!=

用于推进迭代器的前置和后置递增操作符++

用于读取元素的解引用操作符operator*; 以及箭头操作符operator->
```cpp
template<typename T> 
T operator*(const DataContainer<T>& d) // d这里起到this的作用
{
	return *(d.p);
};
...
* forward_iterator  允许读写(如replace,可Operator++, 相当于input_iterator和output_iterator的集合; bidirectional_iterator 可双向移动 可Operator++， Operator--

* random_access_iterator 涵盖随机读写，如p+n, p-n等

随机访问迭代器必须支持

用于比较两个迭代器相对位置的关系运算符, operator>, operator<,

迭代器和一个整数的运算符, operator+, operator-, operator+=
...
如sort通用排序算法，要求其迭代器参数是随即访问迭代器。由于array,deque,vector,string所提供的迭代器都是随机访问迭代器(用于访问内置数组的指针也是)，所以均可使用该sort通用排序算法。而list和forward_list提供的是双向和前向迭代器，因此这两个顺序容器类型不能使用通用的sort排序算法。

* type类型, 迭代器保存的指向元素的信息, 因此迭代器相比指针能保存更多的信息
...
迭代器的作用是用简单的++, --操作屏蔽容器元素内部的复杂操作。迭代器比容器类更底层，容器类的多种函数通过调用迭代器完成。一般的，迭代器的作用是维护节点指针但拥有比指针更丰富的信息, 且algorithm和容器往往持有迭代器(作为处理指向节点的指针)。
...
```cpp
    struct _List_node_base
    {
      ...
      void
      _M_hook(_List_node_base* const __position) _GLIBCXX_USE_NOEXCEPT; // 用于插入节点, 将当前_List_node_base节点插入到position节点前面
...
```cpp
  ...
  template<typename _Tp>
    ...
    // 迭代器指向的节点_List_node_base _M_node, 也是struct _List_iterator维护的内部变量
...
```cpp
  ...
  void
    push_back(const value_type& __x)
    { this->_M_insert(end(), __x); }  // 插入到end()节点前面
	 ...
	 __tmp->_M_hook(__position._M_node);  // tmp插入节点, 这里的__position是迭代器, 而__position._M_node是迭代器维护的节点指针。
       }
