...
* 条款24：若所有参数皆需类型转换, 请为此采用non-member函数(例如operator<<)
...
copy函数，一次Person copy函数，四个string copy函数共六个。 **以上byvalue传值总体成本为六个copy构造函数，六个析构函数。**
...
显然 **以上不论on-the-stack还是on-the-heap，返回引用都很容易导致错误。实际上由于要建立对象，这时候返回引用效率不必直接返回对象好多少。**
...
2. **对于一个public成员变量，如果做出更改甚至取消了它。所有使用它的客户码都会被破坏；对于一个protected成员变量，更改后所有derivedclass会被破坏；对private，没有客户码会被破坏。** 显然使用private更容易维护，客户只管使用对象，而对内部结构改变没有必要知道。
...
一般的，可以将以上clearBrowser函数成为某工具类untity class的一个static member函数，只要它不是WebBrowser的一部分或其friend，就不会影响WebBrower的private成员的封装性。
...
实际上 **以上正是C++标准程序库的组织方式** ， 标准库有大量头文件例如 \<vector> \<alogrithm> \<memory>等，同时每个头文件声明了std命名空间相关机能。当用户使用List只需要include<List>从而只对使用的小部分系统形成了编译相依。而通过声明std可以轻松扩展标准库文件的机能。而class必须整体定义，不能切割为片片段段，**因为class不能直接在文件外访问。**
...
**令class支持隐式转换通常是一个糟糕的注意，一般设置为explicit; 但也有例外，常见的是数值类型。**
...
```cpp
    ...
    result = 2 * oneHalf;    // 错误 
```
上述混合运算只有一半行得通。原因在于不存在2.operator*(oneHalf)。而前者出现了隐式转换int ->Rational, 但是并没有给2转型后的对象实现operator*函数。如果设置构造函数为explicit，2不可以转型为Rational对象, 也就是说都不可以通过编译。

解决上述问题办法， **使用non-member函数解决可能存在的隐式转换问题。**, 变成`operator* (const Rational& lhs, const Rational& rhs)`。如果要访问类内的private成员, 可以在类中将函数声明为friend。
```cpp
    class Rational { 
    public: 
        ... 
...
```
以上，如果要swap两个Widget对象只需要置换其pImpl指针, 但缺省的swap实现相当于`T temp(a); a = b; b = temp;` 会调用`Widget& operator=(const Widget&)`内部的`*pImpl = *(rhs.pImpl); `, 这样会复制三个`Widget`(temp临时对象)还要复制三个`WidgetImpl`对象(`*pImpl = *(rhs.pImpl); `造成的)。

考虑到这是模板写的class, 将std::swap针对Widget特化。当执行swap(widget)时将实际调用指针。但是一般由于内部指针pImpl为private成员，只能被member和friend函数访问。所以我们令Widget声明一个swap的public成员函数做真正的置换工作，然后**将std::swap全特化，令它调用Widget的swap成员函数** 。
```cpp
    class Widget { 
    public: 
        Widget(const Widget& rhs); 
    ...
    namespace std { 
        template<>    // std::swap函数特化 , 如果调用swap(Widget)则调用Widget内部的swap
...
```
以上， **只需要在std中特化swap函数, 再在输入的class实现swap函数就可以实现std::swap的自动调用**。

当Widget和WidgetImpl为class template, 而不是class时。因为C++不支持function对class template的偏特化, 这样就不能用特化解决类模板的问题了, 解决办法是使用重载std::swap, 重新实现一个`void swap (Widget<T>& a, Widget<T>& b) `。

对已经类型的class实施模板特化, 对未知类型的class template实施重载。
```cpp
    template<typename T>
    class WidgetImpl {...}
...
一般而言，重载function template没有问题。但是std内容完全由C++标准委员会决定，重载`std`里头的函数似乎不妥。把swap移出std命名空间。这样如果直接调用`swap`, C++的名称查找法则会找到最合适的`WidgetStuff::swap`而不是`std::swap`。但注意不要在程序写`std::swap`而是直接写`swap`
...
```
但在重载`WidgetStuff`之余, 也要实现模板特化的`std::swap`, 因为编译器会优先调用std特化版的`swap`,如下swap声明为std::swap的特化版或者重载版本。而是置于新的命名空间WidgetStuff内。
```cpp
    namespace WidgetStuff { 
...
``` 
虽然swap函数没有定义在std命名空间内，但是不妨碍C++名称查找规则。即参数为`Widget<T>&`时会优先调用命名空间WidgetStuff的swap，而不是缺省swap。但注意swap前不应该添加任何修饰符，例如std::swap。这时候
**C++会自动挑选最合适的进行调用** 。
...
综上：
  ...
  * 对于 class(非template)，直接对class 进行std::swap特化, 并调用类内部的swap成员函数
  * 调用swap时, 使用using表达式, 不要加std等修饰符
