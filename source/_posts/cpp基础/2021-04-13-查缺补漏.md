---
title: C/C++ 宏定义 内存管理 进程空间 字节对齐(字节序) 弹性数组 const extern
date: 2021-04-13
updated: 2021-04-13
tags: 
  - cpp
...
sizeof(CZeroTest)为4，即弹性数组的大小为0，不占用空间。之所以称为弹性数组，是因为给该类型的指针可以通过malloc或者new的方式给其数组分配一定的空间。弹性数组本身不占用结构体的空间, 但分配内存时需要结构体空间+弹性数组空间。
```cpp
...
弹性数组意味着整个结构体都要分配在堆上。需要对数组内存进行扩容时，你需要对整块内存realloc。
