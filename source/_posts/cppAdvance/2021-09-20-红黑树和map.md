...
map是STL里的一个模板类，用来存放`<key, value>`键值对的数据结构，它的定义如下, 传入三个模板参数, Key, Value和比较器的类型。默认的比较器实现为std::less<_Key>
...
插入`<key, value>`时，map会先通过比较函数地函数对象来比对key的大小，然后根据比对结果进行有序存储。对于作为key的class, 其必须可以支持std::less<Key>, 一种有效的方式是对Class Key重载`operator<`操作符
...
int main(int argc, char* argv[]){
    const auto& cmp = [](const Person &p, const Person &q)
    {
        return (p.age < q.age) || (p.age == q.age && p.name.length() < q.name.length());
    };
    map<Person, int, decltype(cmp)> group(cmp);
    group[Person("Mark", 17)] = 40561;
    group[Person("Andrew",18)] = 40562;
    for (auto ii = group.begin() ; ii != group.end() ; ii++)
        cout << ii->first.name 
...
上面的lamda表达式和sort函数的lamda表达式很相似, 它们都等价于 less<Person>()对象

#### less和greater

less 是一个模板类, 它也可以认为是一种binary_function, 只需要实例化一个参数模板, 它重载了operator<操作符
...
struct hash_name{
	...
	size_t operator()(const Person & p) const{
		/// hash<string>()是对象, hash<string>()()是对象重载的函数
...
map, 自定义key, 一种方式是Key class自行实现opeator<重载。另一种是通过lambda表达式实现less<class>, 并将cmp传入模板参数和构造函数

unordered_map, 自定义key, 一种方式Key class自行实现operator==, 以及hash<class>, 当然简单的还是通过lamda表达式, 需要把实现equal<class>和hash<class>的lamda表达式作为模板参数和构造参数

红黑树核心的要求, 根和叶子是黑色且从任一节点到叶子节点的黑色节点数量相同, 这样保证根节点不同路径节点数量不会超过黑节点两倍, 由于黑节点相同, 这必然是一个平衡树

较AVL树, 红黑树只有插入节点父节点是红色时才可能发生左旋右旋, 减少左右旋转次数